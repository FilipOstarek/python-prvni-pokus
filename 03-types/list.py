...
#owo = {'name': 'Ledecká', 'state': 'CZE', 'number': 100}
#print(type(owo))
...
print(f'\tVypíše všechny hodnoty z mixed_list kromě dvou posledních:{mixed_list[:4]} {mixed_list[4]["name"]}')
# + str(len(mixed_list))
'''
i=0
...
for i in range(4000):
    ...
    if k % 200 == 0:
        hundreds.append(k)
#print(f'{hundreds}')
#print(f'{hundreds[:3]} - První 3, {hundreds[int(len(hundreds))-3:]} - Poslední 3')
del hundreds[:3]
del hundreds[int(len(hundreds))-3:]
#print(f'{hundreds}')
for i in range(50):
    asciii.append(random.choice(string.ascii_uppercase))
for i in range(50):
    if int(asciii.count(asciii[i])) == 1:
        unique.append(asciii[i])
#print(f'{asciii}')
#print(f'{unique}')
print(f'ascii - {asciii}')
print(f'unique - {unique}')
print(f'hundreds - {hundreds}')
del asciii[int(len(hundreds)):]
#print(f'{asciii}')
#print(f'{hundreds}')
...
women = []
ipeople = []
persons += ('Tomas',36,'muž'),('Johana',45,'žena'),('Alex',25,'muž'),('Barbora',29,'žena')
print(persons)
women = list(filter(lambda item: item[2] == 'žena', persons))
#women = [item for item in persons if item[2] == 'žena']
for i in range(len(women)):
    print(f'{women[i][0]}')
    pomlka = '-' * int(len(women[i][0]))
    print(f'{pomlka}')
    pomlka = ''
for i in range(len(persons)):
    '''
    lidi = persons[i][0]
    if lidi.find('i') >= 0 or lidi.find('I') >= 0:
    print("KILLME")
    '''
ipeople = [item for item in persons if item[0].find('i') >= 0 or item[0].find('I') >= 0]
ipeople.sort(key=lambda item: item[0], reverse=True)
print('index;pohlaví;rok;jméno')
for i in range(len(ipeople)):
    print(str(i) + ';' + ipeople[i][2] + ';' + str(ipeople[i][1]) + ';' + ipeople[i][0])
#print(lidi.find('i'))
#print(str(lidi.find('I')) + '\n---')
